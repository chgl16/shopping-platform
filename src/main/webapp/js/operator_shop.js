...
function allCheckFun(){
	shop_item = document.querySelectorAll(".shop_item");
	...
	for(let i=0,len=lahei_btn.length;i<len;i++){
		lahei_btn[i].onclick = function(event){
			...
			yes_btn.onclick = function(){
				...
				shop_item[i].parentNode.removeChild(shop_item[i]);
			}
		}
	}
}
...
function dataToHtml(data){
	...
	$(data.list).each(function(i,val){
		...
		html += '<td><span class="lahei_btn">拉黑</span></td>';
		html += '</tr>';
	});
	return html;
}
